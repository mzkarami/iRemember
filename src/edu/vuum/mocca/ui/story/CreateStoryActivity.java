...
public class CreateStoryActivity extends StoryActivityBase {
	...
	if (Environment.getExternalStorageState().equals(
	    ...
	    Log.d(LOG_TAG, "Media output is mounted");
	} else {
	    Log.d(LOG_TAG, "Media output is not mounted!!!!");
	}
	...
	Uri cameraFileUri = CreateStoryActivity
		.getOutputMediaFileUri(MEDIA_TYPE_IMAGE);

	fragment.imagePath = cameraFileUri;

	// TODO - Add the filename to the Intent as an extra. Use the
	// Intent-extra name
	// from the MediaStore class, EXTRA_OUTPUT
	captureImageIntent.putExtra(MediaStore.EXTRA_OUTPUT, cameraFileUri);

	// TODO - Start a new activity for result, using the new intent and the
	// request
	// code CAMERA_PIC_REQUEST
	this.startActivityForResult(captureImageIntent, CAMERA_PIC_REQUEST);
	...
	File videoFile = CreateStoryActivity
	...
	this.startActivityForResult(captureVideoIntent, CAMERA_VIDEO_REQUEST);
    }
}